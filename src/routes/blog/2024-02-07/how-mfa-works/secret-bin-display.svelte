<script lang="ts">
	...
	let bitRefs: HTMLSpanElement[] = $state(new Array($secretBytes.length * 8));
...
{#each chunks(Array.from($secretBytes), 5) as chunk, chunkIndex}
	<div class={`flex items-center justify-center gap-2 my-2 ${getByteColor(chunkIndex, 1)}`}>
		{#each chunk as byte, byteIndex}
			<span
				ontouchstart={() => pushHoverRange(chunkIndex, byteIndex)}
				onmouseenter={() => pushHoverRange(chunkIndex, byteIndex)}
