<script lang="ts">
	...
	interface Props {
		mfaInitComplete?: boolean;
	}

	let { mfaInitComplete = $bindable(false) }: Props = $props();

	let errorMessage = $state('');
	let codeIsValid = $state(false);

	let secretChunks = $state([$secret]);
	let totpUri = $state('');
	let qrDataUrl = $state('');
	let inputCode = $state('');
	let darkQr = $state(false);
...
<div class="p-4" id="seedInitRoot">
	...
	{#if !codeIsValid}
		...
		<form onsubmit={preventDefault(validateCode)}>
			<ControlledInput
