...
try {
	db.exec(`ALTER TABLE nominations ADD COLUMN metadata TEXT`);
} catch {}
try {
	db.exec(`ALTER TABLE sessions ADD COLUMN rankings TEXT`);
} catch {}
try {
	db.exec(`ALTER TABLE sessions ADD COLUMN expected_participants INTEGER`);
} catch {}
...
const getRawSession = async (sessionId: string): Promise<Session | null> => {
	...
	const nominations = JSON.parse(result.nominations).map(
		(n: { id: string; value: string; metadata: string | null }) => ({
			...n,
...
export const getMyNominations = (sessionId: string, participantId: string): string[] => {
	const result = db
		.prepare('SELECT value FROM nominations WHERE session = ? AND owner = ?')
...
export const getMyVotes = (sessionId: string, participantId: string) => {
	const result = db
		.prepare('SELECT nomination, rank FROM votes WHERE session = ? AND participant = ?')
...
export const getVotedUsers = (sessionId: string): string[] => {
	const result = db
		.prepare('SELECT DISTINCT participant FROM votes WHERE session = ?')
...
export const getNominatorCount = (sessionId: string): number => {
	const result = db
		.prepare('SELECT COUNT(DISTINCT owner) as count FROM nominations WHERE session = ?')
...
const putSession = async (session: SessionDbRecord) => {
	db.prepare(
		'INSERT INTO sessions (id, host_key, mode, expected_participants) VALUES (?, ?, ?, ?)'
...
export const nominate = async (
	...
	db.prepare(
		'INSERT INTO nominations (id, session, owner, value, metadata) VALUES (?, ?, ?, ?, ?)'
	).run(
		crypto.randomUUID(),
		sessionId,
		participantId,
	...
	const sessionData = db
		.prepare('SELECT expected_participants, closed_at FROM sessions WHERE id = ?')
		.get(sessionId) as
		| { expected_participants: number | null; closed_at: number | null }
		| undefined;
	...
	if (sessionData?.expected_participants && !sessionData.closed_at) {
		...
		if (count >= sessionData.expected_participants) {
			...
			console.log(
				`Session ${sessionId} auto-closed after ${count} participants submitted nominations`
			);
		}
	}
};
...
export const closeSession = async (sessionId: string, hostKey: string) => {
	const now = Math.floor(Date.now() / 1000);
	db.prepare('UPDATE sessions SET closed_at = ? WHERE id = ? AND host_key = ?').run(
		now,
		sessionId,
		hostKey
	);
...
export const openSession = async (sessionId: string, hostKey: string) => {
	db.prepare('UPDATE sessions SET closed_at = NULL WHERE id = ? AND host_key = ?').run(
		sessionId,
		hostKey
	);
};
...
export const vote = async (sessionId: string, participantId: string, votes: string[]) => {
	for (let i = 0; i < votes.length; ++i) {
		db.prepare(
			'INSERT INTO votes (session, participant, nomination, rank) values (?, ?, ?, ?)'
	...
	const sessionData = db.prepare('SELECT winner FROM sessions WHERE id = ?').get(sessionId) as
		| { winner: string | null }
		| undefined;

	if (sessionData && !sessionData.winner) {
		const { nominatorCount } = db
			.prepare('SELECT COUNT(DISTINCT owner) as nominatorCount FROM nominations WHERE session = ?')
			.get(sessionId) as { nominatorCount: number };

		const { voterCount } = db
			.prepare('SELECT COUNT(DISTINCT participant) as voterCount FROM votes WHERE session = ?')
...
const finalizeInternal = (sessionId: string) => {
	...
	const votes = rawVotes.map((v) => ({
		participant: v.id,
		votes: JSON.parse(v.participantVotes)
	})) as {
		participant: string;
		votes: {
			id: string;
	...
	db.prepare('UPDATE sessions SET winner = ?, rankings = ? WHERE id = ?').run(
		winner,
...
export const finalize = async (sessionId: string, hostKey: string) => {
	const session = db
		.prepare('SELECT * FROM sessions WHERE id = ? AND host_key = ?')
