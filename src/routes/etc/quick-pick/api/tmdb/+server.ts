    ...
    const movieDetails = await Promise.all(
        searchResults.map(async (movie) => {
            try {
                ...
                if (!r.ok) return { originCountry: undefined, runtime: undefined };
                const details: { origin_country?: string[]; runtime?: number } = await r.json();
                return {
                    originCountry: details.origin_country?.[0],
                    runtime: details.runtime || undefined,
                };
            } catch {
                return { originCountry: undefined, runtime: undefined };
            }
        })
    );
