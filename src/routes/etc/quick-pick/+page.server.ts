...
export const actions = {
	create: async ({ cookies, request }) => {
		...
		const { session, hostId } = await makeSession(
			mode,
			expectedParticipants && !isNaN(expectedParticipants) ? expectedParticipants : undefined
		);
		console.log({ session, hostId });

		cookies.set('quick-pick.hostKey', session.hostKey!, {
			path: `/etc/quick-pick/${session.id}`,
			httpOnly: false
		});

		cookies.set('quick-pick.participantId', hostId, {
			path: `/etc/quick-pick/${session.id}`,
			httpOnly: false
		});
